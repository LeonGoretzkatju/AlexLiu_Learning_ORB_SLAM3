...
void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
    ...
    //set the linear optimization using levenberg algorithm
    g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
    ...
    if(pbStopFlag)
        optimizer.setForceStopFlag(pbStopFlag);//force the optimization to stop
    ...
    */
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        if(pKFi->mnId>maxKFid)
            continue;
        VertexPose * VP = new VertexPose(pKFi);//add the pose vertex
        VP->setId(pKFi->mnId);
        pIncKF=pKFi;
        bool bFixed = false;
        if(bFixLocal)
        {
            bFixed = (pKFi->mnBALocalForKF>=(maxKFid-1)) || (pKFi->mnBAFixedForKF>=(maxKFid-1));//计算关键帧是否会被固定
            if(!bFixed)
                nNonFixed++;//calculate the number of the keyframes need to be optimized
            VP->setFixed(bFixed);
        }
        ...
        if(pKFi->bImu)
        {
            VertexVelocity* VV = new VertexVelocity(pKFi);//add the velocity vertex
    ...
    //若函数调用时处于惯导初始化过程中，则只需要将最后一个关键帧的偏置，作为顶点添加到优化图中
    if (bInit)
    {
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(!pKFi->mPrevKF)//无前一关键帧数据，IMU数据无用
        {
            Verbose::PrintMess("NOT INERTIAL LINK TO PREVIOUS FRAME!", Verbose::VERBOSITY_NORMAL);
            continue;
        }

        if(pKFi->mPrevKF && pKFi->mnId<=maxKFid)//inertisl linked to the previous frame and ID <= maxid
        {
            if(pKFi->isBad() || pKFi->mPrevKF->mnId>maxKFid)
                continue;
            if(pKFi->bImu && pKFi->mPrevKF->bImu)//current and previous frame IMU initialnization is OK
            {
                //更新当前关键帧IMU偏置参数，当前关键帧与前一关键帧，都作为顶点，添加到优化器定义中
                pKFi->mpImuPreintegrated->SetNewBias(pKFi->mPrevKF->GetImuBias());//get the previous IMU bias
                ...
                if (!bInit)
                {
                    //若非处于初始化过程中，相互联系的两个关键帧的偏置都有待优化，若初始化，只优化上述最后一个关键帧的偏置
                ...
                EdgeInertial* ei = new EdgeInertial(pKFi->mpImuPreintegrated);//use the ImuPreintegrated as the edge to connect the vertex
                ...
                g2o::RobustKernelHuber* rki = new g2o::RobustKernelHuber;//use the huber kernel to solve the reprojection error
                ei->setRobustKernel(rki);
                rki->setDelta(sqrt(16.92));

                optimizer.addEdge(ei);//设置鲁棒核参数后，添加到优化图中

                if (!bInit)
                {
                    //关注两个相邻关键帧之间偏置的变化，设置将陀螺仪/加速计偏置连接起来的双向边，添加到优化图中
    ...
    if (bInit)
    {
        //初始化阶段，只关心整体估计的偏置参数，而不是每一帧的参数，使用先验权重，设置对应单向边，添加到优化图中
    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        vPoint->setId(id);
        vPoint->setMarginalized(true);
        optimizer.addVertex(vPoint);//地标点在世界坐标系中位置，作为顶点添加到优化器

        const map<KeyFrame*,tuple<int,int>> observations = pMP->GetObservations();
        //获取可观测到当前地标点的关键帧相关信息，用于建立地标点与关键帧之间的联系

        bool bAllFixed = true;

        //Set edges 对可被观测到的每一个关键帧，获取其信息
        ...
        if(bAllFixed)//if fix all the points ,remove the point set
        {
            optimizer.removeVertex(vPoint);
            vbNotIncludedMP[i]=true;
        }
    ...
    if(pbStopFlag)//if stop the thread
        if(*pbStopFlag)
            return;

    //operate the optimization
    optimizer.initializeOptimization();
    optimizer.optimize(its);


    // Recover optimized data
    //Keyframes get the pose velocity and the bias
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        KeyFrame* pKFi = vpKFs[i];
        if(pKFi->mnId>maxKFid)
    ...
    //Points get the optimized world position and update depth
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        if(vbNotIncludedMP[i])
            continue;
...
//这里对应着论文中的inertial-only MAP estimation
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
{
    ...
    // Set KeyFrame vertices (fixed poses and optimizable velocities)设置每个关键帧对应的节点（包括位姿以及优化的速度），该部分对应于论文中的inertial-only factor-graph
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        KeyFrame* pKFi = vpKFs[i];
        if(pKFi->mnId>maxKFid)//if the id of the keyframe > maxkfid ,jump the loop
            continue;
        VertexPose * VP = new VertexPose(pKFi);//add the pose vertex of each keyframe,correspond to the Ti in the factor graph
        VP->setId(pKFi->mnId);
        VP->setFixed(true);
        optimizer.addVertex(VP);

        VertexVelocity* VV = new VertexVelocity(pKFi);//add the velocity vertex of each keyframe,correspond to the Ti in the factor graph
    ...
    // Biases
    //add the gyrobias vertex,correspond to the Ti in the factor graph
    ...
    if (bFixedVel)
        ...
        VG->setFixed(false);
    optimizer.addVertex(VG);
    //add the accbisa vertex,correspond to the Ti in the factor graph
    ...
    epa->setVertex(0,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VA));
    double infoPriorA = priorA;
    epa->setInformation(infoPriorA*Eigen::Matrix3d::Identity());// set the information matrix
    optimizer.addEdge(epa);// add the edge to connect the vertex
    EdgePriorGyro* epg = new EdgePriorGyro(cv::Mat::zeros(3,1,CV_32F));
    epg->setVertex(0,dynamic_cast<g2o::OptimizableGraph::Vertex*>(VG));
    double infoPriorG = priorG;
    epg->setInformation(infoPriorG*Eigen::Matrix3d::Identity());
    ...
    for(size_t i=0; i<N; i++)
    {
        ...
        {
            pKFi->SetNewBias(b);//update the IMU bias
            if (pKFi->mpImuPreintegrated)
                pKFi->mpImuPreintegrated->Reintegrate();//repeat the preintegrate
        }
        else
            pKFi->SetNewBias(b);
