    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                {
                    // Step 6.1 处于IMU模式并且当前关键帧所在的地图已经完成IMU初始化 进行VI-SLAM的追踪
                ...
                // Initialize IMU here
                // Step 7 当前关键帧所在地图的IMU还没有初始化并且当前模式是IMU模式
        ...
        // Tracking will see that Local Mapping is busy
        SetAcceptKeyFrames(true);//at thie timr the LocalMapping thread can accept the keyframe from the tracking thread
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (mbMonocular)
    {
        minTime = 2.0;// 对应于论文中所说的vision-only map eastimation 下的2 seconds 以及10个相机位姿
        nMinKF = 10;
    }
    else
    {
    ...
    if(mpAtlas->KeyFramesInMap()<nMinKF)//如果atlas地图集中的关键帧<10, return
        return;
    ...
    if(mpCurrentKeyFrame->mTimeStamp-mFirstTs<minTime)//如果短时间内关键帧数量太多，则撤回
        return;
    ...
    IMU::Bias b(0,0,0,0,0,0);//set the initial IMU value of zero to get the good value,used for optimization
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        for(vector<KeyFrame*>::iterator itKF = vpKF.begin(); itKF!=vpKF.end(); itKF++)
        {
            ...
            (*itKF)->SetVelocity(_vel);//将最初有IMU的时间戳以及位姿计算出的速度设置为关键帧的速度
            (*itKF)->mPrevKF->SetVelocity(_vel);
        }

        dirG = dirG/cv::norm(dirG);//正则化 重力在首帧的测量值
        ...
        mRwg = Converter::toMatrix3d(cvRwg);//compute the rotation mastrix 
        mTinit = mpCurrentKeyFrame->mTimeStamp-mFirstTs;//compute the initial timestamp
    }
    else
    {
        mRwg = Eigen::Matrix3d::Identity();//使用单位矩阵进行了initialization
    ...
    std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
    //进行优化，以IMU的位姿作为基准，估计IMU的偏差、scale、以及惯性系与世界系（首帧IMU系）之间的rotation matrix
    ...
    {
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(Converter::toCvMat(mRwg).t(),mScale,true);//recover the scale
        mpTracker->UpdateFrameIMU(mScale,vpKF[0]->GetImuBias(),mpCurrentKeyFrame);//将首帧IMU坐标系对齐到惯性系 
    }
    std::chrono::steady_clock::time_point t3 = std::chrono::steady_clock::now();
    ...
    if (bFIBA)
    {
        if (priorA!=0.f)//if having the prior constraint ,operate the fullinertialBA,minimize to IMU error and reprojection error
            Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, 0, NULL, true, priorG, priorA);
        else
            Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, 0, NULL, false);//corresponding to the visual-inertial map estimation in the paper
    }

    std::chrono::steady_clock::time_point t5 = std::chrono::steady_clock::now();

    // If initialization is OK
    //update the tracking thread to protect the pocal mapping thread
